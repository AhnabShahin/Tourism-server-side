...
async function run() {
  try {
    await client.connect();
      const tourism_db = client.db("tourism");
      const packageCollection= tourism_db.collection("packages");
      const bookingCollection= tourism_db.collection("bookings");

      app.get('/home', async (req, res) => {
        // packageCollection.insertOne({ name: 'John', address: 'Highway 71'});
        res.send({ name: 'shahin', address: 'ahnabshahin@gmail.com'})
      });
...
app.get('/', (req, res) => {
  res.send("Welcome to TOURISM's backend api...");
});
